;(function() {
    ...
    const renderCartItem = ({id, name, attribute, src, price, quantity}) =>{  // добавление html  в корзину
    ...
    const saveCart = () => {//Сохранение в local storage
        localStorage.setItem('cart', JSON.stringify(cart));
    };

    const updateCartTotalPrice = () =>{
        const totalPrice = Object.keys(cart).reduce((acc,id) =>{
            const {quantity, id} = cart[id];
    ...
    const updateCartTotalItemsCounter = () =>{
        ...
        if (cartTotalPriceDOMElement)
        {
            cartItemsCounterDOMElement.textContent = totalPrice;
        }
    };
    ...
    const deleteCartItem = (id) =>{   //удаление из карзины
    ...
    const addCartItem = (data) =>{   //добавление в карзину
        const {id} = data;
    ...
    const updateQuantity = (id, quantity) =>{
        ...
        cartItemQuantityDOMElement.textContent = quantity; //новое кол-во
        cartItemPriceDOMElement.textContent = quantity * cart[id].price; //новая цена
        updateCart();
    };


    const decreaseQuantity = (id) =>{ //Уменьшение числа одной пиццы
    ...
    const increaseQuantity = (id) =>{  //Увеличение числа одной пиццы
        const newQuantity = cart[id].quantity + 1;
        updateQuantity(id,newQuantity);
    };

    const generateID = (string1, string2) =>  //генерация id заказа
    {
